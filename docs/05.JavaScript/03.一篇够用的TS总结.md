...
<!-- more -->
## 一、前言
没写ts之前看[官方文档](https://www.tslang.cn/docs/home.html)，被`那么多`特性吓尿了，实际上写业务用的特性不多。差不多先生，够用就行，这里更多的是，以自己的理解去总结一下常用的ts特性。有理解错误的，`欢迎大哥指正`👏🏻
...
## 三、anyScript
可能因为业务场景或者业务紧张，or某个跑路的大哥省了点功夫，用了typeScript的项目也可能会变成`anyScript`。以下是几种偷懒的方式（`大哥们还没有其他办法`）：
